        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3" data-reveal data-reveal-stagger>
          {serviceList.map((s) => (
            <Link
              key={s.slug}
              href={`/${s.slug}`}
              className="group flex flex-col overflow-hidden rounded-2xl border border-steel-200 bg-white shadow-card transition-all hover:-translate-y-1 hover:border-brass hover:shadow-lift"
            >
              ...
              <div className="flex flex-1 flex-col p-6">
                ...
                <h3 className="mt-1 font-display text-xl font-bold text-ink group-hover:text-crimson">
                  {s.name}
                </h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-steel">{s.short}</p>
                <span className="mt-4 inline-flex items-center gap-1.5 font-display text-sm font-bold uppercase tracking-wide text-crimson">
                  View service
